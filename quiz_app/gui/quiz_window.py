...
from PyQt5.QtWidgets import (
    ...
    QDialog, QSpinBox, QScrollArea, QFrame, QLineEdit
)
...
class QuizWindow(QDialog):
    ...
    def __init__(self, qm: QuestionManager, student_name: str = "", parent=None):
    ...
    def _build_ui(self):
        ...
        # Question counter only - no progress bar or score during quiz
        counter_row = QHBoxLayout()
        self.counter_label = QLabel("")
        self.counter_label.setStyleSheet("color: #7f8c8d; font-size: 12px;")
        counter_row.addWidget(self.counter_label)
        counter_row.addStretch()
        main.addLayout(counter_row)
    ...
    def _show_current_question(self):
        ...
        # Update counter label with current question number
        total_questions = len(self.engine.selected_questions)
        current_num = self.engine.current_index + 1
        self.counter_label.setText(f"Question {current_num} of {total_questions}")
        
        self.question_label.setText(f"Question {current_num}: {question.get('question')}")
    ...
    def _show_results(self):
        """Display quiz results in a scrollable dialog - ONLY AT THE END"""
        ...
        student_label.setStyleSheet("font-size: 14px; font-weight: bold; color: #2c3e50;")
        ...
        summary_layout.addWidget(student_label)
        ...
        # Disable quiz controls and close
        self.submit_btn.setDisabled(True)
        self.next_btn.setDisabled(True)
        self.accept()
...
class MainWithQuiz(QWidget):
    ...
    Now includes student name input.
    """
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Take Quiz")
    ...
    def _build_ui(self):
        ...
        # Student Name Input
        name_group = QGroupBox("Student Information")
    ...
    def start_quiz(self):
        ...
        self.student_name = self.name_input.text().strip()
        if not self.student_name:
            QMessageBox.warning(self, "Name Required", "Please enter your name before starting the quiz.")
            return
        
        # Check if questions exist
        questions = self.qm.get_all_questions_flat()
        if not questions:
            QMessageBox.warning(self, "No Questions", "No questions available. Please contact administrator.")
            return
        
        # Create quiz window and pass question manager and student name
        quiz_win = QuizWindow(self.qm, self.student_name, parent=self)
