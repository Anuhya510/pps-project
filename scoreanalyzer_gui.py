        ...
        if RESULTS_FILE.exists():
            try:
                ...
                for idx, quiz in enumerate(reversed(self.all_quizzes), 1):
                    ...
                    self.quiz_selector.addItem(f"Quiz #{idx} - {date} | Score: {score}/{total} ({percentage:.1f}%)")
            ...
            self.info_label.setText(f"No results file found at: {RESULTS_FILE}\nPlease take a quiz first to generate results.")
            self.all_quizzes = []
    ...
    def load_json(self):
        ...
        try:
            ...
            if isinstance(data, list):
                ...
                # Check if it's a single quiz or has 'quizzes' key
